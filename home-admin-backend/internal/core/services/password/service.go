...
func (s *service) SaveCategory(ctx context.Context, id *uuid.UUID, title string, passwords []domain.PasswordData) (*domain.PasswordCategory, error) {
	c, err := domain.NewCategory(id, title, passwords)
	if err != nil {
		return nil, fmt.Errorf("password service: %w", err)
	}

	if err := s.passwordRepo.SaveCategory(ctx, c); err != nil {
		return nil, err
	}
...
// 		domainP, err := domain.NewPassword(p.ID, catID, p.URL, p.Login, p.Password)
// 		if err != nil {
// 			return nil, fmt.Errorf("password service: %w", err)
// 		}
...
// 	if err := s.passwordRepo.SavePasswords(ctx, domainPasswords); err != nil {
// 		return nil, err
// 	}
